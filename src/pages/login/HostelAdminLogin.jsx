...
const HostelAdminLogin = () => {
    ...
    async function LoginHostelAdmin(e){
        e.preventDefault();
        setLoading(true);
        try{
            const {data} = await axios.post(backendUrl + '/hostel-admin/login', {email, password}, {withCredentials:true});
  ...
  return (
    <div className="bg-purple-100 w-full flex flex-col items-center justify-center min-h-screen">
          <h1 className="text-3xl font-bold mb-6">Hostel Admin Login</h1>
          ...
          { !loading && <div className="w-[50%] bg-white shadow-lg rounded-lg p-6">
                <form onSubmit={LoginHostelAdmin} className="flex flex-col gap-4">
